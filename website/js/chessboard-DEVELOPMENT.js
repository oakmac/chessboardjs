...
;(function () {
  ...
  function assert (assertion, msg) {
    if (window.console && isFunction(window.console.assert)) {
  ...
  function forEachObject (obj, fn) {
    assert(isFunction(fn), 'fn should be a function in forEachObject')
  ...
  // calls fn(itm, idx) for every itm in arr
  function forEachArray (arr, fn) {
    assert(isFunction(fn), 'fn should be a function in forEachArray')
  ...
  // returns the containerEl or false
  function checkContainerArg (containerElOrString) {
    ...
    // did they pass in a <div> DOM node directly?
    if (isElement(containerElOrString) &&
        isString(containerElOrString.tagName) &&
        containerElOrString.tagName.toLowerCase() === 'div') {
      return containerElOrString
    }

    // pass in an id?
    var el = byId(containerElOrString)
    if (isElement(el)) {
      return el
    }

    // pass in a query selector?
    el = qs(containerElOrString)
    if (isElement(el)) {
      return el
    }

    // else error out and return false
  ...
  function constructor (containerElOrString, config) {
    ...
    var containerEl = checkContainerArg(containerElOrString)
    if (!containerEl) return null

    // put an id on the container element if there is not already one
    if (!isString(containerEl.id)) {
      containerEl.id = 'chessboard-container-' + uuid()
    }
    ...
    function calculateSquareSize () {
      var containerWidth = parseInt(containerEl.offsetWidth, 10)
    ...
    // TODO: make this a "run once" function?
    function createElIds () {
      ...
      for (i = 0; i < pieces.length; i++) {
        ...
        sparePiecesElsIds[whitePiece] = 'spare-piece-' + uuid()
        sparePiecesElsIds[blackPiece] = 'spare-piece-' + uuid()
      }
    }
    ...
    function drawPositionInstant () {
      ...
      var piecesEls = qsAll('#' + boardEl.id + ' .' + CSS.piece)
      forEachArray(piecesEls, removeNode)

      // add the pieces from the currentPosition
      // FIXME: use append here instead of innerHTML
      forEachObject(currentPosition, function (itm, idx) {
        var html = buildPieceHTML(itm)
        var squareEl = byId(squareElsIds[idx])
        squareEl.innerHTML += html
      })
    }

    function drawBoard () {
      boardEl.innerHTML = buildBoardHTML(currentOrientation)

      drawPositionInstant()

      if (config.sparePieces) {
        if (currentOrientation === 'white') {
          sparePiecesTopEl.innerHTML = buildSparePiecesHTML('black')
          sparePiecesBottomEl.innerHTML = buildSparePiecesHTML('white')
        } else {
          sparePiecesTopEl.innerHTML = buildSparePiecesHTML('white')
          sparePiecesBottomEl.innerHTML = buildSparePiecesHTML('black')
        }
      }
    }
    ...
    function initDOM () {
      ...
      if (config.sparePieces) {
        sparePiecesTopEl = qs('#' + containerEl.id + ' .' + CSS.sparePiecesTop)
