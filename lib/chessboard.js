...
class Chessboard {
  ...
  #buildPiece(pieceName, hidden, id = `${pieceName}-${uuid()}`) {
  ...
  #animateSquareToSquare(src, dest, piece, completeFn) {
    ...
    const srcSquare = /** @type {HTMLElement} */(this.#squares.get(src));
    const destSquare = /** @type {HTMLElement} */(this.#squares.get(dest));

    // create the animated piece and absolutely position it
    // over the source square
    const animatedPiece = document.body.appendChild(this.#buildPiece(piece, true));
  ...
  #doAnimations(animations, oldPos, newPos) {
    ...
    for (let i = 0; i < animations.length; i++) {
      ...
      if (animation.type === 'clear') {
        const square = /** @type {HTMLElement} */(this.#squares.get(animation.square));
        square.querySelectorAll(`.${ClassNameLookup.piece}`)
          .forEach(x => {
        ...
        if (destination != null) {
          if (sparePieces) {
            const srcSquare = document.getElementById(this.#sparePiecesElsIds[pieceName]);
            if (!srcSquare) {
              throw new Error(`Unable to locate source square with id ${this.#sparePiecesElsIds[animation.piece]}`);
            }
            ...
            piece
              .animate(
                [getJqueryStyleOffset(srcSquare), getJqueryStyleOffset(destination)],
                this.#config.moveSpeed
              )
              .onfinish = () => {
                piece.style.removeProperty('position');
                destination.querySelector(`.${ClassNameLookup.piece}`)?.remove();
  ...
  #drawPositionInstant() {
    ...
    for (const i in this.#currentPosition) {
      ...
      this.#squares.get(i)?.append(this.#buildPiece(this.#currentPosition[i]));
    }
  }
      ...
      for (let j = 0; j < 8; j++) {
        ...
        square.ondrop = (evt) => {
          ...
          if (evt.target != null && evt.dataTransfer != null) {
            const pieceId = evt.dataTransfer.getData("id");
            const source = evt.dataTransfer.getData("source");
            let piece = /** @type {HTMLElement} */(document.getElementById(pieceId));
            piece = source === "spare" ? this.#buildPiece(piece.dataset.piece) : piece;
            piece.dataset.square = /** @type {HTMLElement} */(evt.target).dataset.square;
