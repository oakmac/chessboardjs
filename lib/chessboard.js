...
;(function () {
  ...
  function buildContainerHTML (hasSparePieces) {
    ...
    if (hasSparePieces) {
      html += '<div style="text-align:center;"><div class="{sparePieces} {sparePiecesTop}"></div></div>'
    }

    html += '<div class="{board}"></div>'

    if (hasSparePieces) {
      html += '<div style="text-align:center;"><div class="{sparePieces} {sparePiecesBottom}"></div></div>'
    }
  ...
  function expandConfig (config) {
    ...
    if (!config.hasOwnProperty('pieceTheme') ||
      ...
      config.pieceTheme = '../images/{piece}.png'
    }
  ...
  function constructor (containerElOrString, config) {
    ...
    function spareCountHtml(piece, hidden) {
      ...
      var html = '<span class="spare-count" id="' + sparePiecesCountElsIds[piece] + '"';
      if (hidden) {
        html += ' style="display:none;"'
      }
      return html + '">' + config.spareCounts[piece] + '</span>'
    }

    function buildPieceHTML (piece, hidden, id, is_spare) {
      var html = '<div class="piece-wrapper"><img src="' + buildPieceImgSrc(piece) + '" '
      if (isString(id) && id !== '') {
        html += 'id="' + id + '" '
      }
      html += 'alt="" ' +
    ...
    function buildSparePiecesHTML (color) {
      ...
      for (var i = 0; i < pieces.length; i++) {
        html += buildPieceHTML(pieces[i], false, sparePiecesElsIds[pieces[i]], true)
      }
    ...
    function beginDraggingPiece (source, piece, x, y) {
      ...
      if (isFunction(config.onDragStart) &&
          ...
          (source === 'spare' && (typeof config.spareCounts === 'object' && config.spareCounts[piece] < 1))) {
        return
      }
    ...
    function stopDraggedPiece (location) {
      ...
      if (config.spareCounts && draggedPieceSource === 'spare' && action === 'drop') {
        config.spareCounts[draggedPiece] -= 1;
        var elem = document.getElementById(sparePiecesCountElsIds[draggedPiece]).innerText = config.spareCounts[draggedPiece];
      }
    }
    ...
    widget.resize = function () {
      ...
      // spare pieces
      if (config.sparePieces) {
        $container.css('paddingLeft', ($board.clientWidth - $container.offsetWidth) / 2 + 'px');
      }
