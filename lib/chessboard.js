...
class Chessboard {
  ...
  #validateConfig(config) {
    ...
    const getValidAnimationSpeed = (speed, defaultValue) => {
      if (speed === 'fast') return defaultValue / 2;
      if (speed === 'slow') return defaultValue * 3;
      if (Number.isInteger(speed) && speed >= 0) return speed;
      return defaultValue;
    };

    // animation speeds
    config.appearSpeed = getValidAnimationSpeed(config.appearSpeed, DEFAULT_APPEAR_SPEED);
    config.snapbackSpeed = getValidAnimationSpeed(config.snapbackSpeed, DEFAULT_SNAPBACK_SPEED);
    config.snapSpeed = getValidAnimationSpeed(config.snapSpeed, DEFAULT_SNAP_SPEED);
    config.moveSpeed = getValidAnimationSpeed(config.moveSpeed, DEFAULT_MOVE_SPEED);
    config.trashSpeed = getValidAnimationSpeed(config.trashSpeed, DEFAULT_TRASH_SPEED);
   ...
   * @param {{ [index: string ]: string }} position
   * @returns {any[]}
   */
  #calculateAnimations(position) {
    const animations = [];
    ...
    for (const i in position) {
      if (
        Object.prototype.hasOwnProperty.call(position, i) &&
        Object.prototype.hasOwnProperty.call(currentPosition, i) &&
        currentPosition[i] === position[i]
      ) {
        delete currentPosition[i];
        delete position[i];
      }
    }
    ...
    for (const square in position) {
      if (Object.prototype.hasOwnProperty.call(position, square)) {
        const source = findClosestPiece(currentPosition, position[square], square);
        if (source) {
          animations.push({ type: 'move', source, destination: square, piece: position[square] });
          delete currentPosition[source];
    ...
    for (const square in position) {
      if (Object.prototype.hasOwnProperty.call(position, square)) {
        animations.push({ type: 'add', square, piece: position[square] });
        delete position[square];
      }
    }
    ...
    for (const square in currentPosition) {
      if (
        Object.prototype.hasOwnProperty.call(currentPosition, square) &&
        // do not clear a piece if it is on a square that is the result
        // of a "move", ie: a piece capture
        !Object.prototype.hasOwnProperty.call(squaresMovedTo, square)
      ) {
        animations.push({ type: 'clear', square: square, piece: currentPosition[square] });
  ...
  #doMoveAnimation(animation, onFinishAnimation) {
    ...
    piece
      ...
      .onfinish = () => {
        ...
        this.#pieces.get(destinationName)?.remove();
