...
function squeezeFenEmptySquares (fen) {
  return fen.replace(/1{2,8}/g, ({ length }) => length)
}

function expandFenEmptySquares (fen) {
  return fen.replace(/[2-8]/g, (length) => '1'.repeat(Number(length)))
}
...
function expandConfig (config) {
  ...
  if (
    ...
    config.pieceTheme =
      'https://unpkg.com/@discape/chessboardjs/dist/img/chesspieces/wikipedia/{piece}.png'
  }
...
function constructor (containerElOrString, config) {
  ...
  function validSpareCounts (counts) {
    if (
      ...
      Object.getOwnPropertyNames(counts).length === 0
    ) {
      return false
    }
    ...
    ;['w', 'b'].forEach(function (color) {
      ;['K', 'Q', 'R', 'B', 'N', 'P'].forEach(function (kind) {
        if (!counts.hasOwnProperty(color + kind)) {
          counts[color + kind] = 0
        }
      })
    })
  ...
  function setInitialState () {
    ...
    // spare piece counts
    if (config.hasOwnProperty('spareCounts') === true) {
      if (config.spareCounts === 'default') {
        config.spareCounts = {
          wK: 0,
          wQ: 0,
          wR: 0,
      ...
      } else if (!validSpareCounts(config.spareCounts)) {
        config.spareCounts = false
      }
    } else {
      config.spareCounts = false
    }
  ...
  function createElIds () {
    ...
    for (i = 0; i < pieces.length; i++) {
      ...
      sparePiecesCountElsIds[whitePiece] = 'count' + whitePiece + ' - ' + uuid()
      sparePiecesCountElsIds[blackPiece] = 'count' + blackPiece + ' - ' + uuid()
    }
  }
  ...
  function spareCountHtml (piece, hidden) {
    if (typeof config.spareCounts !== 'object') {
      return ''
    }
    var html =
      '<div style="position:relative"><span class="spare-count" id="' + sparePiecesCountElsIds[piece] + '"'
    if (hidden) {
      html += ' style="display:none;"'
    }
    return html + '">' + config.spareCounts[piece] + '</span></div>'
  }
  ...
  function buildPieceHTML (piece, hidden, id, is_spare) {
    var html =
      '<div class="piece-wrapper"><img alt="' + getPieceName(piece) + '" src="' + buildPieceImgSrc(piece) + '" '
    if (isString(id) && id !== '') {
      html += 'id="' + id + '" '
    }
    html +=
    ...
    if (is_spare) {
      html += '" />' + spareCountHtml(piece, hidden)
    } else {
      html += '" />'
    }

    return interpolateTemplate(html + '</div>', CSS)
  }
  ...
  function beginDraggingPiece (source, piece, x, y) {
    ...
    if (
      ...
      (source === 'spare' &&
        typeof config.spareCounts === 'object' &&
        config.spareCounts[piece] < 1)
    ) {
      return
    }
    ...
    $draggedPiece.attr('src', buildPieceImgSrc(piece)).css({
      ...
      top: y - squareSize / 2,
      transform: `scale(${
        typeof config.scaleDrag === 'number' ? config.scaleDrag : 1
      })`
    })
  ...
  function stopDraggedPiece (location) {
    ...
    if (
      config.spareCounts &&
      draggedPieceSource === 'spare' &&
      action === 'drop'
    ) {
      config.spareCounts[draggedPiece] -= 1
      var elem = (document.getElementById(
        sparePiecesCountElsIds[draggedPiece]
      ).innerText = config.spareCounts[draggedPiece])
    }
  }
  ...
  widget.resize = function () {
    ...
    if (config.sparePieces) {
      $container.css(
        'paddingLeft',
        ($board.clientWidth - $container.offsetWidth) / 2 + 'px'
      )
    }
  ...
  function addEvents () {
    // prevent "image drag"
    $('body').on('mousedown mousemove', '.' + CSS.piece, function (evt) {
      evt.preventDefault()
    })
  ...
  if (config.preloadImages) preloadImages()
  setInitialState()
  initDOM()
  addEvents()
