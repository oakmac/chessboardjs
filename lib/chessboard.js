...
;(function () {
  ...
  function expandConfig (config) {
    ...
    // default for highlight is true
    if (config.highlight !== false) config.highlight = true

    // check if another highlight css classes were provided
    if (config.highlight1 == undefined) config.highlight1 = CSS['highlight1']
    if (config.highlight2 == undefined) config.highlight2 = CSS['highlight1']
  ...
  function constructor (containerElOrString, config) {
    ...
    function removeSquareHighlights () {
      $board
        .find('.' + CSS.square)
        .removeClass(config.highlight1 + ' ' + config.highlight2)
    }
    ...
    function beginDraggingPiece (source, piece, x, y) {
      ...
      if (source !== 'spare') {
        ...
        if (config.highlight) {
          $('#' + squareElsIds[source]).addClass(config.highlight1);
        }
        $('#' + squareElsIds[source])
          .find('.' + CSS.piece)
          .css('display', 'none')
    ...
    function updateDraggedPiece (x, y) {
      ...
      if (validSquare(draggedPieceLocation)) {
        $('#' + squareElsIds[draggedPieceLocation]).removeClass(config.highlight2)
      }

      // add highlight to new square
      if (validSquare(location) && config.highlight) {
        $('#' + squareElsIds[location]).addClass(config.highlight2)
      }
