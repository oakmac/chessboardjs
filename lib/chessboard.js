...
function checkContainerArg(containerElOrString) {
  if (containerElOrString === '') {
    const errorMsg1 = 'Chessboard Error 1001: ' +
      'The first argument to new Chessboard() cannot be an empty string.' +
      '\n\n' +
      'Exiting' + ELLIPSIS;
  ...
  if (container.length !== 1) {
    const errorMsg2 = 'Chessboard Error 1003: ' +
      'The first argument to new Chessboard() must be the ID of a DOM node, ' +
...
class Chessboard {
  ...
  #drawBoard() {
    ...
    if (this.#config.sparePieces) {
      ...
      if (this.#currentOrientation === 'white') {
        /** @type {HTMLElement} */(this.#sparePiecesTop).replaceChildren(...spareBlackPieces);
        /** @type {HTMLElement} */(this.#sparePiecesBottom).replaceChildren(...spareWhitePieces);
      } else {
        /** @type {HTMLElement} */(this.#sparePiecesTop).replaceChildren(...spareWhitePieces);
        /** @type {HTMLElement} */(this.#sparePiecesBottom).replaceChildren(...spareBlackPieces);
      }
    }
...
export { Chessboard, fenToObj, objToFen, validSquare, validPieceCode, validFen, validPositionObject, START_FEN, START_POSITION };
