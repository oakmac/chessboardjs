...
class Chessboard {
   ...
   */
  clear({ useAnimation } = {}) {
    this.setPosition('clear', { useAnimation });
  }
  ...
  move(...args) {
    ...
    for (const i in moves) {
      ...
      if (!this.#position.has(i)) continue;

      const piece = this.#position.get(i);
      this.#position.delete(i);
      if (typeof piece === 'string' && piece.length > 0) {
        this.#position.set(i, piece);
      }
    }

    // update the board
    this.setPosition(this.position, useAnimation);
  }
   ...
   * @returns
   */
  setPosition(position = undefined, { useAnimation = true } = {}) {
    if (!position) {
  ...
  // Control Flow
  // -------------------------------------------------------------------------
  #clearPieces({ useAnimation = true } = {}) {
    for (const [pieceName, piece] of this.#pieces.entries()) {
      this.#pieces.delete(pieceName);
      piece
        .animate([{ opacity: 1 }, { opacity: 0 }], useAnimation ? this.#config.trashSpeed : 0)
        .onfinish = () => {
          piece.remove();
        };
    }
  ...
  #drawBoard({ useAnimation = true } = {}) {
    this.#clearPieces({ useAnimation });
