...
class Chessboard {
  ...
  constructor(containerElOrString, config) {
    ...
    this.#draggedPiece = this.#buildPiece('wP', true, uuid());
    ...
    window.onmousemove = (evt) =>
      this.#isDragging && this.#updateDraggedPiece(evt.pageX, evt.pageY);
    window.onmouseup = () =>
      this.#isDragging && this.#stopDraggedPiece(this.#draggedPieceLocation);
  ...
  #dropDraggedPieceOnSquare(location) {
    ...
    const targetSquareId = this.#squareElsIds[location];
    ...
    const piece = /** @type {HTMLElement} */(this.#draggedPiece.cloneNode());
    piece.style.removeProperty('left');
    piece.style.removeProperty('right');
  ...
  #buildPiece(piece, hidden, id) {
    ...
    pieceEl.onmousedown = (evt) => evt.preventDefault();
    pieceEl.onmousemove = (evt) => evt.preventDefault();
  ...
  #buildSparePieces(color) {
    ...
    return pieces.map(x => {
      const piece = this.#buildPiece(x, false, this.#sparePiecesElsIds[x]);
      piece.onmousedown = (evt) => this.#mousedownSparePiece(evt);
      return piece;
    });
  }
  ...
  #doAnimations(animations, oldPos, newPos) {
    ...
    for (let i = 0; i < animations.length; i++) {
      ...
      if (animation.type === 'clear') {
        ...
        const piece = this.#buildPiece(pieceName, !sparePieces);

        const element = document.getElementById(this.#squareElsIds[squareName]);
        if (element) {
          if (sparePieces) {
            // this.#animateSparePieceToSquare(animation.piece, animation.square, onFinishAnimation3);
            const srcSquare = document.getElementById(this.#sparePiecesElsIds[animation.piece]);
            const destSquare = document.getElementById(this.#squareElsIds[animation.square]);
            if (!srcSquare) {
              throw new Error(`Unable to locate source square with id ${this.#squareElsIds[animation.piece]}`);
            }
            if (!destSquare) {
              throw new Error(`Unable to locate destination square with id ${this.#squareElsIds[animation.square]}`);
            }
            ...
            piece
              ...
              .onfinish = () => {
                piece.style.removeProperty('position');
                destSquare.querySelector(`.${ClassNameLookup.piece}`)?.remove();
                destSquare.prepend(piece);
                onFinishAnimation3();
              };
          } else {
            element.prepend(piece);
            piece
              .animate([{ opacity: 0 }, { opacity: 1 }], this.#config.appearSpeed)
              .onfinish = onFinishAnimation3;
          }
        }
  ...
  #drawBoard() {
    ...
    for (let i = 0; i < 8; i++) {
      ...
      for (let j = 0; j < 8; j++) {
        ...
        square.onmousedown = (e) => this.#mousedownSquare(e);
        square.onmouseenter = (e) => this.#mouseenterSquare(e);
        square.onmouseleave = (e) => this.#mouseleaveSquare(e);
  ...
  #beginDraggingPiece(source, piece, x, y) {
    ...
    this.#draggedPiece = piece instanceof HTMLElement ? piece : this.#buildPiece(piece);
  ...
  #updateDraggedPiece(x, y) {
    ...
    this.#draggedPiece.style.setProperty('left', `${x - this.#squareSize / 2}px`);
    this.#draggedPiece.style.setProperty('top', `${y - this.#squareSize / 2}px`);
    this.#draggedPiece.style.setProperty('display', 'block');
  ...
  #mousedownSquare(evt) {
    ...
    const { square } = evt.currentTarget.dataset;
  ...
  #touchstartSquare(evt) {
    // do nothing if we're not draggable
    if (!this.#config.draggable) return;

    // do nothing if there is no piece on this square
    const { square } = evt.target.dataset;
    if (!validSquare(square)) return;
    if (!Object.prototype.hasOwnProperty.call(this.#currentPosition, square)) return;
